...
from .fpn_fsd_head import FPN_FSDHead, FAM

@HEADS.register_module()
class FPN_FSD_SEPHead(FPN_FSDHead):
    ...
    def __init__(self, feature_strides, sep_cfg, **kwargs):
        super(FPN_FSD_SEPHead, self).__init__(feature_strides, **kwargs)
        ...
        self.fam_module_novel =  FAM(in_channels=128, out_channels=128)
        for i in range(len(feature_strides)):
            ...
            self.cls_seg_novel = nn.Conv2d(128, self.num_classes, kernel_size=1)
    ...
    def forward_train(self, inputs, img_metas, gt_semantic_seg, train_cfg):
        ...
        """
        base_fg, base_output, novel_fg, novel_output = self.forward(inputs)
        # print(torch.unique(gt_semantic_seg))

        losses = self.losses(novel_fg, novel_output, gt_semantic_seg)
        return losses
    ...
    def forward_test(self, inputs, img_metas, test_cfg):
        ...
        """
        fs_base, base_output, fg_novel, novel_output = self.forward(inputs)
        output = base_output
    ...
    def forward(self, inputs):
        ...
        fg_output_novel, output_novel = self.fam_module_novel(output_novel)

        output = self.cls_seg(output)
        output_novel = self.cls_seg_novel(output_novel)

        return fg_output, output, fg_output_novel, output_novel
