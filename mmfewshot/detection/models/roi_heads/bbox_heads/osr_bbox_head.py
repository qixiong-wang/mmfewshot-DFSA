...
class OSRBBoxHead(ConvFCBBoxHead):
    ...
    def orthogonal_decompose(self, feats, bases_b, bases_n=None):
        ...
        if bases_n is not None:
            # 分成两半，前半只算base的，获得前景score和背景score
            # 后半为了算novel的score和背景score，必须都算了
    ...
    def forward_all(self, x_cls):
        ...
        # self.base_ft.append(out_fg_base.squeeze())
        # self.novel_ft.append(out_fg_novel.squeeze())   
        ...
        # feats_src_bg = torch.sum(feats_novel, dim=1).contiguous().view(B, C)
    ...
    def get_base_bboxes(self,
        ...
        # base_det_bboxes, base_det_labels, base_det_inds = multiclass_nms(base_bboxes, 
        #                                                 base_scores,
    ...
    def get_novel_bboxes(self,
        ...
        # novel_det_bboxes, novel_det_labels, novel_det_inds = multiclass_nms(novel_bboxes, 
        #                                                       novel_scores,
        #                                                       cfg.novel_score_thr, cfg.nms,
        #                                                       cfg.novel_max_per_img, return_inds=True)
    ...
    def get_bboxes(self,
        ...
        # base_bool = (det_bboxes[:, -1] > 0.95) * (det_labels < 15)
        # base_select_id = (base_inds[base_bool] / 15).long()
        # base_select_label = det_labels[base_bool]
        # base_ft = self.base_ft.pop()
        # base_select_ft = base_ft[base_select_id, base_select_label]
        # base_select_ft = torch.cat([base_select_ft, base_select_label.unsqueeze(1)], dim=-1)
        # self.base_ft.append(base_select_ft)
        ...
        # novel_bool = (novel_det_bboxes[:, -1] > 0.95) * (novel_det_labels + self.n_base < 20)
        # novel_select_id = (novel_inds[novel_bool] / 5).long()
        # novel_select_label = novel_det_labels[novel_bool]
        # novel_ft = self.novel_ft.pop()
        # novel_select_ft = novel_ft[novel_select_id, novel_select_label]
        # novel_select_ft = torch.cat([novel_select_ft, novel_select_label.unsqueeze(1) + self.n_base], dim=-1)
        # self.novel_ft.append(novel_select_ft)
        # torch.save(torch.cat([torch.cat(self.base_ft), torch.cat(self.novel_ft)]), 'ft.pt')
        return det_bboxes, det_labels



@HEADS.register_module()
class OSRBBoxHeadWOBC(ConvFCBBoxHead):
    """BBOxHead for `TFA <https://arxiv.org/abs/2003.06957>`_.
    ...
    def __init__(self, 
                 ...
                 *args,
                 **kwargs) -> None:
        super(OSRBBoxHeadWOBC, self).__init__(
            num_shared_convs=0,
            num_shared_fcs=2,
            num_cls_convs=0,
            num_cls_fcs=0,
    ...
    def forward(self, x):
        ...
        if not self.training:
            guide_score = cls_score[:len(x) // 2]
            base_ans = guide_score.argmax(-1)
            self.novel_id = base_ans > 14
            self.base_id = ~self.novel_id
            cls_score = torch.cat([cls_score[:len(x) // 2][self.base_id], cls_score[len(x) // 2:][self.novel_id]])
    ...
    def loss(self,
        ...
        if cls_score is not None:
            if self.fewshot:
                # sample_cls_score = torch.cat([cls_score[:, :self.n_base], cls_score[:, self.n_base + 1:]], dim=1)
                sample_cls_score = cls_score[B // 2:]
                avg_factor = max(torch.sum(label_weights > 0).float().item(), 1.)
            else:
                avg_factor = max(torch.sum(label_weights > 0).float().item(), 1.)
            ...
            if cls_score.numel() > 0:
                ...
                if self.custom_activation:
                    ...
                    if self.fewshot:
                        losses['acc'] = accuracy(cls_score[B // 2:], labels)
                    else:
                        losses['acc'] = accuracy(cls_score, labels)
        if bbox_pred is not None:
            bg_class_ind = self.num_classes
    ...
    def get_base_bboxes(self,
                        ...
                        cfg=None):
        if self.custom_cls_channels:
            base_scores = self.loss_cls.get_activation(cls_score[self.novel_id, self.n_base + 1:])
        else:
            base_scores = F.softmax(cls_score[self.base_id], dim=-1)
            
            base_bboxes = bboxes[self.base_id, :]
            base_bboxes_remain_id = (rois[self.base_id, 1:] != 0).sum(-1) != 0
        ...
        if len(base_bboxes) == 0:
            return base_bboxes[:, :5], base_bboxes[:, 0]
        base_det_bboxes, base_det_labels = multiclass_nms(base_bboxes, 
                                                        base_scores,
                                                        cfg.score_thr, cfg.nms,
                                                        cfg.max_per_img)
    ...
    def get_novel_bboxes(self,
                        ...
                        cfg=None):
        if self.custom_cls_channels:
            novel_scores = self.loss_cls.get_activation(cls_score[self.novel_id])
        else:
            novel_scores = F.softmax(cls_score[self.novel_id], dim=-1)
            
            novel_bboxes = bboxes[self.novel_id, :]
            novel_bboxes_remain_id = (rois[self.novel_id, 1:] != 0).sum(-1) != 0
    ...
    def get_bboxes(self,
        ...
        if self.fewshot:
            novel_det_bboxes, novel_det_labels = self.get_novel_bboxes(rois[B // 2:], cls_score, bboxes[B // 2:], cfg)
            novel_inds = novel_det_labels > 14
            novel_det_bboxes = novel_det_bboxes[novel_inds]
            novel_det_labels = novel_det_labels[novel_inds]
            det_bboxes = torch.cat([det_bboxes, novel_det_bboxes])
            det_labels = torch.cat([det_labels, novel_det_labels])
