...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs='+',
        default=['decode_head.scale_heads.0.0.conv', 'decode_head.scale_heads.1.0.conv', 'decode_head.scale_heads.2.0.conv', 'decode_head.scale_heads.2.2.conv', 'decode_head.scale_heads.3.0.conv', 'decode_head.scale_heads.3.2.conv',  'decode_head.scale_heads.3.4.conv',
...
# layer4.0.conv2.org_module.weight,
def copy_checkpoint(param_name, is_weight, checkpoint):
    ...
    if weight_name in checkpoint['state_dict']:
        ...
        # 不要del 有lora读取lora 没有lora就读取原始
        # del checkpoint['state_dict'][weight_name]
    ...
    if args.method == 'remove':
        ...
        for idx, (param_name,
            ...
            random_init_checkpoint(param_name, False, tar_size, checkpoint,
                                   args)
        for stage_num in range(args.frozen_stages, 4):
            for block_num in range(r101_settings[stage_num]):
                for conv_num in range(1, 4):
                    args.lora_name.append('backbone.layer{}.{}.conv{}'.format(stage_num + 1, block_num, conv_num))
                args.lora_name.append('backbone.layer4.0.downsample.0')
